...
This was done during assignment 2.
...
#| echo: FALSE
# Modell 5: Estimer effekten på kombinert Geo, Year, NUTS2-nivå
plm5 <- plm(gini_nuts3 ~ GDP_per_capita, data = Panel_Data, index = c("geo", "year"), model = "within", effect = "twoways") %>% 
  coeftest(vcov. = vcovHC, type = "HC1", save = TRUE)
```
...
We have ended up with 4 panel estimates, but we would like to comment that we also chose a 5 panel estimate.
The reason why we only ended up with 4 panel estimates was because models 4 and 5 got the same results.
Model 4 was specified with fixed effects for geographical area (Geo), year (Year) and NUTS2 level (id_nuts0).
Model 5, where we limited fixed effects to only geographic area (Geo) and year (Year), and omitted NUTS2 level.

Surprisingly, the analysis revealed that the results from Model 4 and Model 5 were identical.
Despite having different compositions of fixed effects, both models produced the same estimates for the effect of GDP_per_capita on gini_nuts3.
This phenomenon can be attributed to the remaining effects of Geo and Year already encompassing what is typically handled by id_nuts0.
As a result, there was no significant difference in the results between the two models.

To avoid unnecessary duplication of results and to keep the analysis concise, we have chosen to include only one of the models, namely Model 4, in the final analysis.
This decision is based on the fact that Model 4 provides a more comprehensive representation of fixed effects, and the results from Model 5 do not add new insights to our research questions.

comment on @tbl-six

In the context of fixed-effects panel estimation models, we explored the impact of GDP per capita on regional inequality, incorporating different combinations of fixed effects---Country, Year, NUTS2, and their various combinations.
