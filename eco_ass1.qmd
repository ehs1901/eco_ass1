...
for (country_code in country_codes) {
  # Filter the dataset for each country
  data_country <- data_2010 %>%
    filter(id_nuts0 == country_code)
  
  if (nrow(data_country) > 0) {  # Check if there is data for the country
    # Create a regression model for the country
    regression_model <- lm(gini_n2 ~ GDP_per_capita, data = data_country)
