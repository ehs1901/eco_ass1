...
Comment: as we can see, both Spain and the Netherlands have a downward slope, the Netherlands a steeper slope than Spain.
If we look at Greece, on the other hand, we see that the line is going up.
...
#| echo: FALSE
library(readr)
highway <- read_csv("motorvei.csv", show_col_types = FALSE)
innovation <- read_csv("innovation.csv", show_col_types = FALSE)
farm <- read_csv("gard.csv", show_col_types = FALSE)
```
...
We use the "select" command to select the columns we need as we are only interested in obs value, year and geo/id_nuts2 code.
...
```{r}
# Create a list of regression models for Spain, the Netherlands, and Greece
regression_models_3countries <- list()

# Filter the dataset for each country and check for data availability
data_es <-  merged_innovation %>% filter(id_nuts0 == "ES")
data_nl <- merged_innovation %>% filter(id_nuts0 == "NL")
data_el <- merged_innovation %>% filter(id_nuts0 == "EL")

# Perform regression for Spain
regression_model_es <- lm(gini_n2 ~ newfirms, data = data_es)
regression_models_3countries$ES <- regression_model_es

# Perform regression for the Netherlands
regression_model_nl <- lm(gini_n2 ~ newfirms, data = data_nl)
regression_models_3countries$NL <- regression_model_nl

# Perform regression for Greece
regression_model_el <- lm(gini_n2 ~ newfirms, data = data_el)
regression_models_3countries$EL <- regression_model_el

summary(regression_model_es)
summary(regression_model_nl)
summary(regression_model_el)
...
```{r}
rm(data, data_el, data_es, data_nl, farm, highway, innovation, main_data2, merged_data, merged_innovation,model_summaries,NO_GDP, NO_merged, subset_data, subset_data_el)
```
